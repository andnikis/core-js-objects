...
function compareObjects(obj1, obj2) {
  ...
  for (let i = 0; i < keys1.length; i += 1) {
    const commonKey = keys1[i];
    if (keys2.indexOf(commonKey) < 0 || obj1[commonKey] !== obj2[commonKey]) {
      return false;
    }
  }
...
function makeWord(lettersObject) {
  ...
  for (let i = 0; i < entries.length; i += 1) {
    ...
    for (let wordIndex = 0; wordIndex < entry[1].length; wordIndex += 1) {
      const [letter, positions] = entry;
      word[positions[wordIndex]] = letter;
    }
  }
  return word.join('');
}
...
function sortCitiesArray(arr) {
  const result = arr.sort((a, b) => {
    const equal = a.country.localeCompare(b.country);
    if (equal !== 0) {
      return equal;
    }
